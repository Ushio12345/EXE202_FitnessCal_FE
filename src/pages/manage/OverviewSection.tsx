    ...
    const fetchUserStats = async () => {
      try {
        ...
        if (response.data.isSuccess) {
          setUserStats(response.data.userStatistics);
        } else {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="p-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Người dùng đăng ký</p>
              {loading ? (
                <div className="animate-pulse">
        ...
        <Card className="p-6 relative overflow-hidden">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{formatCurrency(metrics.averageRevenue)}</p>
            </div>
            <Camera className="w-5 h-5 text-gray-400" />
          </div>
          <div className="flex items-center text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Revenue Trend Chart */}
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Phân bố doanh thu hàng tháng trong 6 tháng qua</h3>
          
          <div className="flex items-center justify-between">
            {/* Donut Chart Placeholder */}
            <div className="flex-shrink-0">
              <div className="w-32 h-32 rounded-full border-8 border-purple-200 flex items-center justify-center bg-gradient-to-br from-purple-100 to-purple-50">
                <div className="text-center">
                  <p className="text-sm font-medium text-gray-600">Total</p>
                  <p className="text-lg font-bold text-purple-600">+7.8B</p>
                </div>
              </div>
            </div>
                ...
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center text-xs">
                      <span className="text-gray-500 mr-1">{item.percentage}%</span>
                      {item.isPositive ? (
        ...
        {/* Recent Activities */}
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Giao dịch mới nhất của người dùng</h3>
